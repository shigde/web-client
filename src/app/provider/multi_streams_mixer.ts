...
// This is a copy from
// @see: https://github.com/muaz-khan/MultiStreamsMixer/blob/master/MultiStreamsMixer.ts
export class MultiStreamsMixer {
  ...
  // @ts-ignore
  gainNode: GainNode | any;


  constructor(_arrayOfMediaStreams: MediaStream[] , elementClass = 'multi-streams-mixer') {
  ...
  private setSrcObject(stream: any, element: any ) {
  ...
  getMixedStream() {
    ...
    if (mixedVideoStream == undefined) {
      ...
      if (mixedAudioStream) {
        mixedAudioStream.getTracks().filter(function (t: any) {
          return t.kind === 'audio';
        }).forEach((track: any) => {
  ...
  private getMixedVideoStream() {
    ...
    let capturedStream = this.canvas.captureStream() || this.canvas.mozCaptureStream();
    let videoStream = new MediaStream();
    capturedStream.getTracks().filter(function (t: any) {
      return t.kind === 'video';
    }).forEach((track:any) => {
  ...
  appendStreams(streams: any) {
    ...
    streams.forEach((stream: any) => {
      if (stream.getTracks().filter(function (t: any) {
        return t.kind === 'video';
      }).length) {
        let video = this.getVideo(stream);
        //video['stream'] = stream;
